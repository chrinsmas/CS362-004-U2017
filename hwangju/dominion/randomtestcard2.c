...
int main(int argc, char** argv)
{
    ...
    for(j = 0; j < 1000; j++)
    {
        ...
        retValue = cardEffect(great_hall, 0, 0, 0, &game, cardPos, &bonus);

        if(retValue != 0)
        {
            printf("\nError in great hall card, returned an error incorrectly in run %d\n", j);
            errorPresentThisRun = 1;
        }

        if(game.handCount[player] != numCardsInHand)
        {
            printf("\nError in great hall card, did not gain a card in run %d\n", j);
            printf("Previous hand count: %d Post great hall card: %d \n", game.handCount[player], numCardsInHand);
            errorPresentThisRun = 1;
        }

        if(game.numActions != numActionsPrev + 1)
        {
            printf("\nError in great hall card, did not gain one actions in run %d\n", j);
            errorPresentThisRun = 1;
        }
    ...
    if(errorPresent == 0)
    {
        printf("\nNo error found in great hall random tests.\n");
    }
    else
    {
        errorTotal = (errorTotal / 1000) * 100;
        printf("\nErrors found in %d%% runs of great hall random tests.\n", errorTotal);
    }
